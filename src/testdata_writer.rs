...
pub fn write_parquet(
    ...
    while start < end {
        ...
        if ttypes::NESTED {
            ...
            match &mut pw {
                ParquetWriter::FileWriter(ref mut writer) => {
                    write_parquet_row_group_nested(writer, start, group_end)
                }
        ...
        println!(
            "End of group {} in {:?} has value {} is account {} with amount {}",
            ng,
            now.elapsed(),
            last,
            ttypes::make_label(last),
            ttypes::find_amount(last)
        );
        start += group_size;
        ng += 1;
    }
...
fn write_parquet_row_group_nested<W: io::Write>(
    ...
    while let Some(mut col_writer) = row_group_writer.next_column().unwrap() {
        ...
        match col_nr {
            0 => {
                ...
                let values = values.into_iter().fold(Vec::new(), |mut vals, v| {
            ...
            1 => {
                ...
                let values: Vec<i32> = (start..end).map(|i| ttypes::find_amount(i)).collect();

                col_writer
                    .typed::<Int32Type>()
                    .write_batch_with_statistics(
                        &values,
                        Some(&definition_levels[0..]),
                        Some(&repetition[0..]),
                        Some(&-499),
                        Some(&500),
                        Some(distinct),
...
//        let reorder: Box<impl Fn(u64) -> u64> = if UNORDERED_RG {
        ...
        match col_nr {
            0 => {
                let values: Vec<i64> = rec_ids.map(|i| (i as i64)).collect();
                col_writer
                    .typed::<Int64Type>()
                    .write_batch_with_statistics(
                        &values,
            ...
            1 => {
                let values: Vec<_> = rec_ids
                    .map(|i| ByteArray::from(ttypes::make_label(i).as_bytes().to_vec()))
                    .collect();
                col_writer
                    .typed::<ByteArrayType>()
                    .write_batch_with_statistics(
            ...
            2 => {
                let distinct = 1000 as u64;
                let values: Vec<i32> = rec_ids.map(ttypes::find_amount).collect();
                col_writer
                    .typed::<Int32Type>()
                    .write_batch_with_statistics(
                        &values,
            ...
            3 => {
                let values: Vec<i64> = rec_ids.map(ttypes::find_time).collect();
                col_writer
                    .typed::<Int64Type>()
                    .write_batch_with_statistics(
                        &values,
